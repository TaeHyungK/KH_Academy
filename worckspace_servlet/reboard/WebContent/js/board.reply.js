$(document).ready(function(){
				...
				if(count < 0 || list == null){
					...
					$(list).each(function(index,item){
						...
						if($('#user_id').val() && $('#user_id').val()==item.id){
							...
							output += ' <input type="button" value="수정" data-num="'+item.re_num+'" data-id="'+item.id+'" class="modify-btn">';
							output += ' <input type="button" value="삭제" data-num="'+item.re_num+'" data-id="'+item.id+'" class="delete-btn">';
						}else{
							//로그인 하지 않거나 작성자 id와 다를 경우
	...
	$(document).on('keyup', 'textarea', function(){
		...
		if(inputLength>300){//300자를 넘은 경우
