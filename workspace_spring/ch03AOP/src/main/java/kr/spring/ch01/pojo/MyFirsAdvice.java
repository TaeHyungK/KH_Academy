...
public class MyFirsAdvice {
	 ...
	 * After Advice				대상 객체의 메서드를 실행하는 도중 예외가 발생했는지의 여부와 상관없이 메서드
	 * 							실행 후 공통 기능을 실행(try~catch~finally의 finally블럭과 비슷)
	...
	public void afterThrowing(Throwable ex) {
		//메서드 호출이 예외가 던졌을 때 동작하는 어드바이스
		System.out.println("Hello AfterThrowing! **예외가 생기면 나온다. 예외 : " + ex);
	}
	
	public void after() {
		//예외가 발생해도 실행됨, 메서드 종료 후에 동작하는 어드바이스
	...
	public String around(ProceedingJoinPoint joinPoint) throws Throwable{
		...
		try {
			...
			System.out.println("Hellow Around after! **메서드가 호출된 후에 나온다! 반환된 객체 : " + s);
		}
		return s;
	}
}
