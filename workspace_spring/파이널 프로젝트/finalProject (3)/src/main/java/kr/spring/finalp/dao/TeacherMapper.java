...
public interface TeacherMapper {
	...
	//강사 전체 정보
	@Select("SELECT * FROM teacher WHERE a_num = #{a_num}")
	...
	//강사 상세정보
	@Select("select * from teacher where t_num = #{t_num}")
	...
	@Select("SELECT * FROM (SELECT * FROM teacher ORDER BY t_good DESC) WHERE rownum <= 6")
